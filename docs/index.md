# Documentation Index

## Source-of-truth order

When documents appear to conflict, follow this order:

1. Accepted architecture decisions and current migrations
2. `docs/architecture/v1-build-readiness.md`
3. `docs/architecture/technical-architecture.md`
4. `docs/architecture/repository-bootstrap.md`
5. Current domain specifications
6. Implementation plans

Do not rely on chat history after a decision has been consolidated into these files.

## Architecture

- [V1 Build Readiness](architecture/v1-build-readiness.md)
- [Technical Architecture](architecture/technical-architecture.md)
- [Repository Bootstrap](architecture/repository-bootstrap.md)
- [Consolidated Object Model](architecture/object-model.md)
- [Database Schema](architecture/database-schema.md)
- [State Transitions](architecture/state-transitions.md)

## Domains

- [Customer, Lead, Estimate, and Quote](domains/sales-and-intake.md)
- [Project and Shared Job](domains/project-and-job.md)
- [Material Delivery Detail](domains/material-delivery-detail.md)
- [Material Load and Material Load Item](domains/material-load.md)
- [Dump Trailer Rental Detail](domains/dump-trailer-rental-detail.md)
- [Disposal Load](domains/disposal-load.md)
- [Job Charge](domains/job-charge.md)
- [Invoice Architecture](domains/invoice.md)
- [Payment, Allocation, Credit, and Refund](domains/payment.md)
- [Communications and Customer Experience](domains/communications-and-customer-experience.md)

## Implementation

- [Build Sequence](implementation/build-sequence.md)
- [V1 Sprint Roadmap](implementation/sprints.md)
- [Acceptance Journeys](implementation/acceptance-journeys.md)
- [Sprint 1.6.0 Material Delivery Operations Plan](implementation/plans/2026-08-03-sprint-1-6-0-material-delivery-operations.md)
- [Sprint 1.7.0 Invoicing and Payments Plan](implementation/plans/2026-08-04-sprint-1-7-0-invoicing-payments.md)
- [Sprint 1.8.0 Dump Trailer Rental Plan](implementation/plans/2026-08-09-sprint-1-8-0-dump-trailer-rental.md)
- [Sprint 1.9.0 Communications and Customer Experience Plan](implementation/plans/2026-08-09-sprint-1-9-0-communications-customer-experience.md)
- [Codex Handoff](implementation/codex-handoff.md)

## Runbooks

- [Local Development](runbooks/local-development.md)
