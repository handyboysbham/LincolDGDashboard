import { Inject, Injectable } from "@nestjs/common";
import { createHash, createHmac, timingSafeEqual } from "node:crypto";

import { ServerConfigService } from "../config/server-config.service.js";

const uuidPattern = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;

export interface ParsedContractToken {
  linkId: string;
  secret: string;
  tenantId: string;
}

@Injectable()
export class ContractTokenService {
  public constructor(
    @Inject(ServerConfigService) private readonly configuration: ServerConfigService,
  ) {}

  public creationKeyHash(key: string): string {
    return sha256(`contract-send:v1:${key}`);
  }

  public create(input: { contractId: string; linkId: string; tenantId: string }): {
    hash: string;
    token: string;
  } {
    const secret = createHmac("sha256", this.configuration.value.objectStorage.publicLinkSigningKey)
      .update(`contract-link:v1:${input.tenantId}:${input.linkId}:${input.contractId}`)
      .digest("base64url");
    return { hash: sha256(secret), token: `cv1.${input.tenantId}.${input.linkId}.${secret}` };
  }

  public matches(secret: string, expectedHash: string): boolean {
    const actual = Buffer.from(sha256(secret), "hex");
    const expected = Buffer.from(expectedHash, "hex");
    return actual.length === expected.length && timingSafeEqual(actual, expected);
  }

  public parse(token: string): ParsedContractToken | undefined {
    const [version, tenantId, linkId, secret, extra] = token.split(".");
    if (
      version !== "cv1" ||
      !tenantId ||
      !uuidPattern.test(tenantId) ||
      !linkId ||
      !uuidPattern.test(linkId) ||
      !secret ||
      secret.length < 32 ||
      extra !== undefined
    )
      return undefined;
    return { linkId, secret, tenantId };
  }
}

function sha256(value: string): string {
  return createHash("sha256").update(value).digest("hex");
}
